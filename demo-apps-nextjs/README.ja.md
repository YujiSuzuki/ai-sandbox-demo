...
> ⚠️ **雛形段階:** 現時点ではシークレットファイルを読むルートハンドラ1本だけの最小構成で、`demo-apps/` のような一通りのウォークスルーにはなっていません。この最小構成自体は、Docker ビルド・HostMCP 経由のログ/ヘルスチェック・ブラウザ表示まで一通り動作確認済みです(下記クイックスタート参照)。
